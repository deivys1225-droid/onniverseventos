...
const ConciertosLiveEmitirPage = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setCardTitle(state?.config?.title ?? state?.draftDefaults?.title ?? "Concierto Live");
      setLoading(false);
    };
    void load();
  }, [user?.id]);
  ...
  const canOpenEmitter = Boolean(user?.id && (emitStatus?.canEmit || (testMode && emitStatus?.hasAccess)));
